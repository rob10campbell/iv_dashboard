...
bean_colors = [ "#a8b448", "#dbc649", "#c8a472", "#b8844e", "#9f6e48"] #, "#652d17" ]  
bean_labels = [ "Color", "Smooth", "Shiny", "Uniform"]
colors = [
...
# --- IV spokes + blank markers (section 1 customized) ---
angles = []
for i in range(num_sections):
    ...
    if i == 0:
        ...
        for v in range(1, num_spokes + 1):
            ...
            # Add spoke line (always black)
            fig.add_trace(go.Scatter(
                x=[0, max_radius_iv * np.cos(angle)],
                y=[0, max_radius_iv * np.sin(angle)],
                mode="lines",
                line=dict(color=spoke_color, width=1.5),
                showlegend=False
            ))
            ...
            for lvl in range(1, num_levels + 1):
                ...
                if v == 1:
                    # first spoke: colored markers by level
                    color = bean_colors[(lvl - 1) % len(bean_colors)]
                    bean_width = 2
                else:
                    # other spokes: black markers
                    color = "black"
                    bean_width = 0.8
